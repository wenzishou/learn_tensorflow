...
def main(_):
  ...
  train_step = tf.train.FtrlOptimizer(0.5, l1_regularization_strength=0.05).minimize(cross_entropy)
  ...
  whe = tf.equal(W, zero)
  whe_cast = tf.cast(whe, tf.float32)
  su = tf.reduce_sum(whe_cast)
  print(sess.run(su))
